...
final class DemoSeeder extends Seeder
{
    ...
    {
        $admin = User::query()->create(['first_name' => 'Admin', 'last_name' => 'istrator', 'email' => 'av@serviscentrs.lv', 'phone' => '+37120000001', 'password' => '090545', 'role' => UserRole::Admin, 'email_verified_at' => now('UTC')]);
        $manager = User::query()->create(['first_name' => 'Fleet', 'last_name' => 'Manager', 'email' => 'manager@anoteh.local', 'phone' => '+37120000002', 'password' => 'password', 'role' => UserRole::Manager, 'email_verified_at' => now('UTC')]);
        $viewer = User::query()->create(['first_name' => 'Read Only', 'last_name' => 'Viewer', 'email' => 'viewer@anoteh.local', 'password' => 'password', 'role' => UserRole::Viewer, 'email_verified_at' => now('UTC')]);
        $volvo = Vehicle::query()->create(['registration_number' => 'AN-1201', 'vin' => 'YV2RT40A1KB123456', 'make' => 'Volvo', 'model' => 'FH 500', 'year' => 2022, 'status' => VehicleStatus::Active, 'category' => VehicleCategory::Truck, 'body_type' => 'tractor', 'fuel_type' => FuelType::Diesel, 'inspection_until' => now()->addDays(30), 'octa_until' => now()->addMonths(6), 'commissioned_on' => '2022-06-15', 'responsible_user_id' => $manager->id]);
        $scania = Vehicle::query()->create(['registration_number' => 'AN-2207', 'vin' => 'YS2R4X20005512345', 'make' => 'Scania', 'model' => 'R450', 'year' => 2020, 'status' => VehicleStatus::Active, 'category' => VehicleCategory::Truck, 'body_type' => 'curtain_sider', 'fuel_type' => FuelType::Diesel, 'inspection_until' => now()->addDays(14), 'octa_until' => now()->addDays(7), 'commissioned_on' => '2020-03-10', 'responsible_user_id' => $manager->id]);
        $van = Vehicle::query()->create(['registration_number' => 'AN-0314', 'vin' => 'W1V9076351P654321', 'make' => 'Mercedes-Benz', 'model' => 'Sprinter', 'year' => 2023, 'status' => VehicleStatus::InService, 'category' => VehicleCategory::Van, 'body_type' => 'panel_van', 'fuel_type' => FuelType::Diesel, 'inspection_until' => now()->addYear(), 'octa_until' => now()->addDays(3), 'commissioned_on' => '2023-09-01', 'responsible_user_id' => $viewer->id]);
